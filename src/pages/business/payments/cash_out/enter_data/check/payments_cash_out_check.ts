...
@Component({
  selector: 'page-business-payments-cash-out-check',
  templateUrl: 'payments_cash_out_check.html',
})
export class BusinessPaymentsCashOutCheckModal {
  userAuth: boolean;
  userData: any;
  token: string;
  typeID: number;
  ...
  sendKey(complete = () => {}) {
    this.userMainService.getBusinessPaymentsCashOutCheck(this.token, this.key).then((result: any) => {
